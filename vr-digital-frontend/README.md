...
For access from another device, set `VITE_API_URL` to the backend's reachable
address, for example `http://192.168.1.20:5000/api`, rather than using
`localhost`. Client, lawyer, and existing admin accounts can sign in from any
reachable IP address.

Complete frontend for **WakeelHub** — a lawyer booking platform inspired by zorservices.co.
