...
const sendWithSmtp = async ({ to, subject, text, html }) => {
  const info = await createTransporter().sendMail({
    from: process.env.SMTP_FROM || `"WakeelHub" <${process.env.SMTP_USER}>`,
    to,
    subject,
    text,
    html,
...
export const sendOtpEmail = async (to, code, purpose = 'verify') => {
  ...
  const subject = `Your WakeelHub OTP: ${code}`;
  const text = `Your one-time password for ${purposeText} is: ${code}\n\nThis code expires in 10 minutes.\nIf you did not request this, ignore this email.`;
  const html = `
      <div style="font-family: Inter, Arial, sans-serif; max-width: 480px; margin: 0 auto; padding: 24px;">
        <h2 style="color: #f59e0b; margin-bottom: 8px;">WakeelHub</h2>
  ...
  if (resend) {
    try {
      const { data, error } = await resend.emails.send({
        from: process.env.RESEND_FROM || 'WakeelHub <onboarding@resend.dev>',
        to,
        subject,
        text,
        html,
      ...
      if (!error) return { success: true, messageId: data?.id };
      if (!hasSmtpConfig()) {
  ...
  if (hasSmtpConfig()) {
    try {
      ...
      console.log(`\n==========================================`);
      console.log(`🔑 [DEV OTP BACKUP] Verification code for ${to}: ${code}`);
      console.log(`==========================================\n`);
      if (process.env.NODE_ENV !== 'production') {
        return { success: true, devMode: true, messageId: 'dev-fallback' };
      }
      throw smtpErr;
    }
  }
