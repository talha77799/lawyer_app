...
export const registerAsLawyer = async (req, res) => {
  try {
    ...
    if (!name || !email || !password || !req.file) {
      return res.status(400).json({ success: false, message: 'Name, email, password and qualification certificate or degree file required' });
    }
    ...
    const lawyer = await User.create({
      ...
      qualificationDocument: `/uploads/qualifications/${req.file.filename}`,
      bio: bio || '',
      verified: false,
      online: false,
    });
