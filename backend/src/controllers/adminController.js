...
const userFields = [
  'name', 'phone', 'city', 'role', 'isActive', 'verified', 'online', 'bio',
  'specialization', 'experience', 'fee', 'languages', 'education', 'barCouncil',
  'location', 'availability', 'matricSchool', 'intermediateCollege', 'lawInstitution',
  'casesHandled', 'casesCleared', 'bankProvider', 'bankAccountNumber', 'qualificationDocument'
];
...
export const listResource = async (req, res) => {
  ...
  if (req.params.resource === 'appointments') {
    recordsQuery = recordsQuery
      .populate('lawyer', 'name email avatar phone city')
      .populate('client', 'name email avatar phone city');
  } else if (req.params.resource === 'cases') {
    recordsQuery = recordsQuery
      .populate('lawyer', 'name email avatar phone city specialization')
      .populate('client', 'name email avatar phone city');
  }
