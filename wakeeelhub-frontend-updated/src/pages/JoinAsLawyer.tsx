...
export default function JoinAsLawyer() {
  ...
  const [, setLawyerPhoto] = useState<File | null>(null)
  const [form, setForm] = useState({
    name: '', email: '', phone: '', city: 'Lahore', gender: 'Male',
    specialization: '', experience: '', barCouncil: '', matricSchool: '',
    intermediateCollege: '', lawInstitution: '', casesHandled: '', casesCleared: '', bankProvider: '', bankAccountNumber: ''
  })
  ...
  return (
    <div className="container" style={{ padding: '2rem 1.25rem', maxWidth: 640 }}>
      ...
      <p style={{ color: 'var(--text-muted)', marginBottom: '1.5rem' }}>
        Share your details and our team will guide you through onboarding, verification, and listing on WakeelHub.
      </p>
      ...
      <form onSubmit={handleSubmit} className="card" style={{ padding: '1.75rem' }}>
        ...
        <div className="form-group">
          ...
          <input name="matricSchool" required value={form.matricSchool} onChange={handleChange} placeholder="School name" />
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
          <div className="form-group">
            <label>Intermediate college</label>
            <input name="intermediateCollege" required value={form.intermediateCollege} onChange={handleChange} placeholder="College name" />
          </div>
          <div className="form-group">
            <label>Law university / institution</label>
            <input name="lawInstitution" required value={form.lawInstitution} onChange={handleChange} placeholder="LLB institution" />
          </div>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
          <div className="form-group">
            <label>Cases handled</label>
            <input name="casesHandled" type="number" min="0" required value={form.casesHandled} onChange={handleChange} />
          </div>
          <div className="form-group">
            <label>Cases successfully cleared</label>
            <input name="casesCleared" type="number" min="0" required value={form.casesCleared} onChange={handleChange} />
          </div>
        </div>
        <div className="form-group">
          <label>Payment bank / wallet</label>
          <select name="bankProvider" required value={form.bankProvider} onChange={handleChange}>
            <option value="">Select provider</option>
            {['EasyPaisa', 'JazzCash', 'HBL', 'UBL', 'MCB', 'Meezan Bank', 'Bank Alfalah', 'Other'].map((provider) => <option key={provider}>{provider}</option>)}
          </select>
        </div>
        <div className="form-group">
          <label>Bank account number for receiving payments</label>
          <input name="bankAccountNumber" required value={form.bankAccountNumber} onChange={handleChange} placeholder="Account number / IBAN" />
        </div>
        <div className="form-group">
          <label>Lawyer profile photo</label>
          <input name="avatar" type="file" required accept="image/png,image/jpeg" onChange={(e) => setLawyerPhoto(e.target.files?.[0] || null)} />
          <small style={{ color: 'var(--text-muted)' }}>PNG or JPG up to 5 MB. This photo will appear on your lawyer profile.</small>
        </div>
        <div className="form-group">
